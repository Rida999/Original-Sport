...
import { deleteSupplier, listSuppliers, saveSupplier } from "@/lib/data";
...
function Suppliers() {
  ...
  const { data } = useQuery({
    queryKey: ["suppliers"],
    queryFn: async () => listSuppliers() as Promise<Supplier[]>,
  });
  ...
  const save = useMutation({
    mutationFn: async () => {
      ...
      await saveSupplier({ data: { id: editing?.id, values: payload } });
    },
    onSuccess: () => {
  ...
  const del = useMutation({
    mutationFn: async (id: string) => deleteSupplier({ data: { id } }),
    onSuccess: () => {
        ...
        <Dialog open={open} onOpenChange={setOpen}>
          ...
          <DialogContent className="max-w-lg">
            ...
            <div className="space-y-3">
              {(["company_name", "contact_person", "phone", "email", "address"] as const).map(
                (k) => (
            ...
            <DialogFooter>
              ...
              <Button onClick={() => save.mutate()} disabled={!form.company_name.trim()}>
                Save
              </Button>
            </DialogFooter>
          </DialogContent>
              ...
              <tr key={s.id} className="hover:bg-muted/30">
                ...
                <td className="p-3">
                  <div className="flex gap-1 justify-end">
                    ...
                    <Button size="icon" variant="ghost" onClick={() => setDelId(s.id)}>
            ...
            {(data?.length ?? 0) === 0 && (
              <tr>
                <td colSpan={5} className="p-10 text-center text-muted-foreground">
                  No suppliers. Only admins can manage suppliers.
                </td>
              </tr>
            )}
          </tbody>
